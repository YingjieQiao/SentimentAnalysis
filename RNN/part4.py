...
def build_vocab(
    data: List[List[str]], use_unk: bool = False, ratio=1.0
) -> Dict[str, int]:
    """
...
def train(
    ...
    for epoch in range(nepoch):
        if epoch % evaluate_loss_after == 0:
            ...
            print(
                "Training loss after num_examples_seen=%d epoch=%d: %f"
                % (num_examples_seen, epoch, loss)
            )
            # Adjust the learning rate if loss increases
            Y_pred_id = [model.predict(sentence) for sentence in X_test_id]
            Y_pred_str = [
                [output_id2token[id] for id in sentence] for sentence in Y_pred_id
            ]
            os.makedirs("../RU/result/%s/" % (runname), exist_ok=True)
            save_prediction(
                "../RU/result/%s/dev_epoch%d.out" % (runname, epoch),
                X_test_str,
                Y_pred_str,
            )
...
X_train_id = [
    [token2id(word, input_token2id) for word in sentence] for sentence in X_train_str
]
...
X_test_id = [
    [token2id(word, input_token2id) for word in sentence] for sentence in X_test_str
]
losses = train(
    model,
    X_train_id,
